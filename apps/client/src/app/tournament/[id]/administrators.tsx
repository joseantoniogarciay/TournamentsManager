...
import {
  ...
  removeTournamentAdministratorRequest,
...
export default function TournamentAdministratorsScreen() {
  ...
  const load = useCallback(
    (force = false) => {
      ...
      void Promise.all([
        force ? refreshTournament(id) : loadTournament(id),
        listTournamentAdministratorUsernames(id),
      ])
  ...
  const remove = async (username: string) => {
    ...
    try {
      await removeTournamentAdministratorRequest(id, username);
    ...
    <>
      <Stack.Screen
        options={{
          ...
          title: t("tournament_administrators"),
          ...(!usesLiquidGlassNavigation
            ? {
                ...
                headerRight: () =>
                  canAddAdministrator
                    ? navigationButton(
                        () => router.push(`/tournament/${id}/administrators/add`),
          ...
          {canAddAdministrator ? (
            <Stack.Toolbar placement="right">
              <Stack.Toolbar.Button
                ...
                icon="plus"
                onPress={() => router.push(`/tournament/${id}/administrators/add`)}
              />
            </Stack.Toolbar>
          ) : null}
        </>
          ...
          >
            {administrators.length === 0 ? (
              <View style={styles.emptyState}>
                <Text color="secondary">{t("tournament_administrators_empty")}</Text>
              </View>
            ) : null}
