...
import {
  ...
  transferTournamentOwnershipRequest,
...
export default function TransferTournamentScreen() {
  ...
  const close = () => {
    ...
    else router.replace(id ? `/tournament/${id}` : "/");
  };
  const transfer = async (username: string) => {
    if (!id || transferring) return;
    setTransferring(username);
    try {
      await transferTournamentOwnershipRequest(id, username);
  ...
  const helper =
    query.length < minimumQueryLength
      ? t("tournament_administrator_search_minimum").replace("{count}", String(minimumQueryLength))
