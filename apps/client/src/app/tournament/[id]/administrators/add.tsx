...
import {
  assignTournamentAdministratorRequest,
  TournamentAdministratorConflictError,
  listTournamentAdministratorUsernames,
...
export default function AddTournamentAdministratorScreen() {
  ...
  useEffect(() => {
    if (!id) return;
    void listTournamentAdministratorUsernames(id)
          ...
          show({
            ...
            message:
              error instanceof UserSearchRateLimitedError
                ? t("tournament_administrator_search_rate_limited")
  ...
  const close = () => {
    ...
    else router.replace(id ? `/tournament/${id}/administrators` : "/");
  };
  const assign = async (username: string) => {
    if (!id || assigning) return;
    setAssigning(username);
    try {
      await assignTournamentAdministratorRequest(id, username);
      close();
    } catch (error) {
      if (error instanceof TournamentAdministratorConflictError) {
        show({ kind: "generic-error", message: t("tournament_administrator_self_assignment") });
      } else {
        const failure = getRequestFailure(error);
        show({ kind: failure.kind, message: t(failure.messageKey) });
      }
  ...
  const helper =
    administrators === undefined
      ...
      : query.length < minimumQueryLength
        ? t("tournament_administrator_search_minimum").replace(
            "{count}",
            String(minimumQueryLength),
          )
        : !searching && visibleResults.length === 0
          ? t("tournament_administrator_search_empty")
          : undefined;
  ...
  return (
    <>
      ...
      <Screen topInset="navigation-bar">
        ...
        >
          <TextField
            accessibilityLabel={t("tournament_administrator_username")}
            autoCapitalize="none"
            autoFocus
            onChangeText={(value) => setQuery(value.toLowerCase())}
            placeholder={t("tournament_administrator_username_placeholder")}
            value={query}
          />
