...
| `POST /v1/tournaments`, equipos, inicio, cancelación, resultado y finalización; `GET /v1/tournaments/{tournamentId}` | Operaciones PostgreSQL estáticas del ciclo del torneo                                                     | La entrada inválida —incluido deporte ausente/desconocido, tanteo de baloncesto empatado o penaltis incompatibles— usa `validation.rejected`; los rechazos de negocio distinguen `tournament.forbidden`, `tournament.not_found`, y conflictos cerrados de inicio, equipos, retirada, resultado, cancelación o finalización. Deporte, tanteos e IDs no se exportan como atributos. Sin limitador específico. Los fallos técnicos y cancelaciones conservan las categorías seguras comunes en el span raíz.                                                                                                                                                                                                                             |
| Inicio mixto y `POST /v1/tournaments/{tournamentId}/stages/elimination/start`                                        | Transacción PostgreSQL de composición, clasificación congelada y generación del cuadro                    | El éxito crea la liga o confirma la transición atómica. JSON o parámetros inválidos usan `validation.rejected`; una composición o plantilla incompatible usa `tournament.configuration_rejected`; una transición prematura, repetida o sin suficientes equipos no retirados usa `tournament.stage_transition_conflict`; autorización y ausencia conservan `tournament.forbidden` y `tournament.not_found`. No se exportan equipos, grupos, cantidades, resultados, retiradas ni IDs. No hay límite de tasa específico. Cada fallo técnico usa `database.*` o `request.failed`, el timeout `request.timeout` y la cancelación `request.cancelled`, sin error bruto y sin spans por validación, clasificación, CTE o generación de IDs. |
