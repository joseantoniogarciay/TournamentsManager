...
| `POST /v1/tournaments`, equipos, inicio, cancelación, resultado y finalización; `GET /v1/tournaments/{tournamentId}` | Operaciones PostgreSQL estáticas del ciclo del torneo                                                     | La entrada inválida usa `validation.rejected`; los rechazos de negocio distinguen `tournament.forbidden`, `tournament.not_found`, y conflictos cerrados de inicio, equipos, retirada, resultado, cancelación o finalización. Sin limitador específico. Los fallos técnicos y cancelaciones conservan las categorías seguras comunes en el span raíz.           |
